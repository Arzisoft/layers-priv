...
lesson: "Module 3: Information Retrieval and Selection — Study Guide"
type: "course_lesson"
post_id: 106578132
space_id: 24391596
source: "https://the-faction.mn.co/posts/106578132"
updated: "2026-08-28T15:46:54Z"
---
...
1. "Build a retrieval test set of 20 real user questions, each mapped to the passage that answers it, and report precision and recall for our current settings."
1. "Add a re-ranking step: retrieve 20 candidates by vector similarity, re-rank against the query, inject the top 5."
1. "Log every retrieval: the query, chunks returned, similarity scores, and whether any score fell below our threshold."
1. "Update the system prompt so the model answers only from the provided passages and says the sources do not cover it when they do not."

## Common Mistakes

1. Chunking by fixed character count through sentence boundaries, producing fragments no query matches well.
1. Trusting similarity scores as proof of relevance, when nearest merely means least far away.
1. Cranking up top-k to fix recall and flooding the window until precision collapses.
1. Never building a test set, so retrieval quality is a feeling instead of a number.
1. Injecting chunks with no source labels or delimiters, leaving the model to guess where documents begin.
1. Having no failure path, so the model improvises whenever retrieval comes back empty or irrelevant.
...
---
_Source: https://the-faction.mn.co/posts/106578132_
