...
lesson: "Module 4: Memory and Conversation Design — Study Guide"
type: "course_lesson"
post_id: 106578135
space_id: 24391596
source: "https://the-faction.mn.co/posts/106578135"
updated: "2026-08-28T15:47:30Z"
---
...
1. "Direct the system to summarize conversation history older than 2,000 tokens into a digest that preserves decisions, facts, and open tasks, and drop the original messages."
1. "At session end, extract durable facts and preferences from this transcript into the user profile, and show me the diff before saving."
1. "At session start, inject the user profile and the last session summary above the conversation, clearly labeled as memory with dates."
1. "Add an expiry review: flag stored memories older than 90 days for confirmation instead of injecting them as current fact."

## Common Mistakes

1. Replaying entire history forever, until the window is all small talk and the system forgets its instructions.
1. Storing everything, which makes recall a search problem through noise instead of a lookup of signal.
1. Summarizing away the wrong things: keeping pleasantries, losing the budget figure the client committed to.
1. Injecting stale memory as current fact, so the system confidently references a plan that changed last month.
1. Building persistent memory for a tool that runs one-off tasks and needed none.
1. Never labeling injected memory, so the model cannot distinguish stored history from the live conversation.
...
---
_Source: https://the-faction.mn.co/posts/106578135_
