...
lesson: "Layer 6: Cloud & Compute — Study Guide"
type: "course_lesson"
post_id: 102892121
space_id: 23777123
source: "https://the-faction.mn.co/posts/102892121"
...
**What "compute" means:** Compute is processing power—the actual CPU time and memory your app uses to do work. Loading a webpage, processing a form, resizing an image—these all use compute. More compute means faster results but higher costs. Your job is finding the balance:

enough power to keep things fast, not so much that you're wasting money.

**Free tiers and their limits:** Most cloud platforms offer a free tier—a certain amount of compute, storage, and data transfer at no cost. This is perfect for getting started, but there are hard limits.

If your app suddenly gets popular and exceeds those limits, you start getting charged. Know where those limits are before your app goes viral.
...
Compute vs. storage: Your cloud bill shows high compute charges but low storage charges.

What does this tell you about how your app is using resources?
...
Ready? Take the [Cloud & Compute Exam →](https://the-faction.mn.co/posts/the-foundation-layer-6-cloud-compute-exam)
...
> PURPOSE
> 
> Cloud & Compute covers where backend work runs: serverless functions, workers, queues, cron jobs, containers, and scheduled or triggered tasks.
> 
> PRINCIPLE
> 
> Cloud work must be secure, limited, observable, cost-aware, and safe to retry.
> 
> WHAT THIS SECTION OWNS
> 
> Two things nothing else in the protocol owns: the cost and limits of running compute, and asynchronous execution (work that runs not in direct response to a user click). Security, secrets, auth, webhook verification, and public exposure are enforced here but defined in #2, #4, #5, and #8 — this section references them, it does not restate them.
> 
> COMPUTE MODEL
> 
> After deployment the app runs on rented machines you do not own; code runs when triggered, and you pay per execution. Use serverless for small, variable, event-based work (APIs, webhooks, image resizing, emails, scheduled jobs). Use a dedicated or long-running server when the app needs constant compute, long tasks, special control, or predictable always-on capacity. Serverless functions sleep when idle and have a cold-start delay on the next call — latency-critical always-on paths favor a long-running server. Do not let AI choose a compute model without explaining cost, limits, scaling, and failure behavior.
> 
> COST & LIMITS (the core of this section)
> 
> Serverless does not mean free — cost grows with executions, runtime, memory, retries, and data transfer. Know the current limits of the hosting plan: invocations, runtime, bandwidth, build minutes, storage, image optimization, logs, and external API calls. Before launch, know what happens at the limit: throttling, failure, pause, overage billing, or forced upgrade. Set billing alerts on day one. Check the usage dashboard weekly; if the bill spikes, find which service grew first. Cache data that rarely changes. Do not call external APIs, AI models, or heavy functions on every page load unless necessary. Optimize images before delivery. Avoid runaway loops, unlimited retries, oversized files, and huge queries. Ask what the system costs at 1,000, 10,000, and 100,000 users.
> 
> ASYNC EXECUTION — JOBS, QUEUES, CRON
> 
> Use background jobs for slow, heavy, delayed, scheduled, or retryable work. Every job must be safe to retry: repeated runs must not double-create, double-charge, double-send, or corrupt data (idempotency law in #2). Permanent failures must stop retrying and land in a dead-letter queue or failure log — never retry forever, never vanish silently. No task may leave data half-updated; use transactions or safe state changes when multiple steps must succeed together (#2, #3).
> 
> SERVERLESS + DATABASE
> 
> Serverless functions scale out horizontally and each instance opens its own database connections. In front of a traditional database, use a connection pooler — or high traffic will exhaust the database's connection limit and it will reject requests. This failure only appears under load, so design for it before launch.
> 
> RELIABILITY
> 
> Every external call has a timeout. Every important task logs start, success, failure, retry, and permission denial (logging detail in #12). A task with unclear identity, permission, config, or scope must stop, not guess.
> 
> ACCESS & SECRETS (enforced here, defined elsewhere)
> 
> Every cloud task that touches private data is a protected gate and must respect Auth (#4), Backend/API (#2), Database (#3), and RLS (#8): it must know what triggered it, what it may do, and which user/team/project/account/record/file it may access. Store secrets only in secure environment settings; never hardcode keys, tokens, or credentials; never expose privileged cloud credentials to the frontend; give each service the smallest permission it needs (secret handling in #5). Verify and de-duplicate webhooks before trusting them (#2).
> 
> EXPOSURE
> 
> Expose only cloud endpoints that must be public. Never expose debug endpoints, internal jobs, admin functions, logs, or test tools (#5).
> 
> TESTING
> 
> Test cloud tasks directly, not only through the frontend: allowed access, denied access, missing secrets, wrong environment, invalid payloads, duplicate jobs, failed dependencies, timeouts, retries, large inputs, and production-like volume.
> 
> AI WORKFLOW
> 
> When asking AI to build cloud work, specify: trigger, input, output, access rule, data scope, secrets, environment, timeout, retry behavior, duplicate prevention, failure destination, logs, cost limits, and test cases. Ask AI to explain the cost model and current platform limits. Working cloud code is not production-ready until usage, security, failure, and cost behavior are verified.
> 
> SHIP GATE — do not ship unless all are true
> 
> ☐ Compute model chosen with cost, limits, and failure behavior explained
> 
> ☐ Free-tier limits known; billing alerts set; cost-at-scale estimated
> 
> ☐ Jobs safe to retry; permanent failures dead-lettered, not lost
> 
> ☐ Connection pooler in place if serverless fronts a traditional database
> 
> ☐ External calls time out; task lifecycle logged (#12)
> 
> ☐ Secrets server-side only; least-privilege per service (#5)
> 
> ☐ Webhooks verified and de-duplicated (#2)
> 
> ☐ No private data or task bypasses Auth (#4), Backend (#2), or RLS (#8)


---
_Source: https://the-faction.mn.co/posts/102892121_
