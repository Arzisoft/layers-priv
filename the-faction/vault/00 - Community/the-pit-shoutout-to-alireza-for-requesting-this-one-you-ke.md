...
source: "https://the-faction.mn.co/posts/106568257"
---

# Shoutout to Alireza for requesting this one. You keep patching v1 because v2 sou

Shoutout to Alireza for requesting this one. You keep patching v1 because v2 sounds like a rewrite. It is not. Today I walk through the real signal that tells you it is time for v2, how to run v1 and v2 side by side without shutting anything down, and why every API without a sunset policy becomes permanent technical debt. If you have been bolting endpoints onto v1 for months and your documentation cannot keep up, this is the conversation.

**PROMPT:** Direct your AI: "Perform an API versioning readiness audit with three components: (1) Breaking change inventory. Audit every endpoint in my current API. For each endpoint, identify any field names, response structures, authentication flows, error formats, or behaviors I would change if backward compatibility were not a constraint. Flag every endpoint where the current implementation differs from what the documentation describes. Categorize each change as cosmetic, structural, or breaking. The breaking changes define the scope of v2. (2) Versioned routing architecture. Design a versioned API routing layer where v1 and v2 run side by side behind the same domain. Route requests based on URL path versioning or header-based versioning. Both versions should share authentication and middleware but maintain independent endpoint logic. Build the routing layer so new versions can be added without modifying existing version code. (3) Sunset policy and migration. Add sunset headers to every v1 endpoint response that include a deprecation date and a link to the v2 migration guide. Build a v1 usage dashboard that tracks which clients are still calling v1 endpoints, how frequently, and which endpoints they use most. Generate a migration guide that maps every v1 endpoint to its v2 equivalent with request and response format differences documented."

---

## Video transcript

_deepgram auto-captions (Mighty) · 2m21s_
